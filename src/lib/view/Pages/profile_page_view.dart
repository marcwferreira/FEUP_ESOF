...
class ProfilePageViewState extends UnnamedPageView {
  ...
  List<Widget> childrenList(BuildContext context) {
    ...
    list.add(ProfileInfoCard(name: name, email: email));
    list.add(Padding(padding: const EdgeInsets.all(5.0)));
  ...
  ///Function to test Profile Preview card
  Widget getProfilePreview(){
    return ListView(shrinkWrap: false, children: [ProfileInfoCard()]);
  }

  /// Returns a widget with the user's profile info (Picture, name and email).
  Widget profileInfo(BuildContext context) {
    return StoreConnector<AppState, Future<File>>( key: Key("info_page"),
